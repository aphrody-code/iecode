...
namespace IECODE.Core.Formats.Criware.CriFs.Definitions;
...
public interface ICpkReader : IDisposable
{
    ...
    /// True if the file will need to be decompressed with CriLayla.
    /// </param>
    /// <returns>Extracted data.</returns>
    ReadOnlySpan<byte> ExtractFileNoDecompression(ref CpkFile file, out bool needsDecompression);
