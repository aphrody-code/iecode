...
#pragma warning restore IL2026
    ...
    Parallel.ForEach(filesToCopy, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, file =>
    {
      ...
      long current = Interlocked.Increment(ref copiedCount);
      // Report progress periodically to avoid flooding the UI thread
      if (current % 10 == 0 || current == totalFilesToCopy)
      {
          CGeneralUtils.ReportProgress((int)current, (int)totalFilesToCopy, "Copying Files");
      }
    });
